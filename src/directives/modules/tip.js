...
const addTip = function (el, {value}, vnode){
  ...
  const maxWidth = Math.max(500, value.length)
  const tipHeight = value.length / (maxWidth / 12) * 18 + 30  // 文字总数 / （一行多少字） * 字高 + 内边距
  const isReachBottom = _.get100vh() - bottom < tipHeight // 距离底部小于100px
  ...
  divObj.setAttribute("style", `${isReachBottom ? `transform-origin:top center;transform:translateY(-${height + 5}px) rotateX(180deg);` : ''}left:${left}px;top:${divTop}px;max-width:${maxWidth}px;min-width:100px;background:#464c5b;white-space:normal;z-index:1000`);
  divObj.innerHTML = `

...
export const showpdf = function (pdfUrl) {
  PDFJS.getDocument({url:pdfUrl, rangeChunkSize:65536, disableAutoFetch:false}).promise.then(pdfDoc=>{
    this.eyeReportPdf.totalPage = pdfDoc.numPages // pdf的总页数
    console.log(pdfDoc.numPages)
    this.pdfDoc = pdfDoc
    // 获取第1页的数据
    for(let i = 1; i <= this.eyeReportPdf.totalPage; i++) {
...
export const renderPage = function (dom, num) {
  this.pdfDoc.getPage(num).then(page =>{
    ...
    canvas.style.width = fatherWidth - 15 + "px"
    canvas.style.height = fatherWidth * 1.415 + "px"
  ...
  const observer = new IntersectionObserver(entries => {
    entries.forEach(async item => {
      ...
      if(intersectionRatio > 0) {
        console.log('触底加载')
        if(this.eyeReportPdf.page < this.eyeReportPdf.totalPage) {
          this.eyeReportPdf.page += 1
        } else {
          this.eyeReportPdf.isShowLoadMore = false
        }
      }
    })
  }, {
